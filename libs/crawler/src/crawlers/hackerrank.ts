...
export class HackerRankCrawler implements Crawler<HackerRankUrlParams> {
  static urlPatterns = [
    {
      type: "challenges_url",
      regexp: pathToRegexp(
  ...
  getUrlParams(url: string): HackerRankUrlParams {
    ...
    for (const pattern of HackerRankCrawler.urlPatterns) {
  ...
  async fetchProblem(url: string) {
    ...
    if (type === "contest_url") {
      ...
      const _name = _(".challenge-page-label-wrapper .ui-icon-label.page-label")
        .text()
        .trim();
      if (!_name) throw new Error(INVALID_PROBLEM_URL_ERROR);
      name = _name;
