...
export default function MedalsTab({ userStats }: { userStats: any }) {
  ...
  return (
    <Box>
      ...
      >
        {[100, 250, 500, 750, 1000].map((val: any) => {
          return (
            ...
            >
              <Box sx={{ opacity: userStats?.totalSolved && userStats.totalSolved >= val ? 1 : 0.2 }}>
                {medalMap[val].icon}
              </Box>
              <Text
                mt="sm"
      ...
      <Modal opened={open} onClose={() => setOpen(false)} title={<Title order={4}>{medalMap[medal]?.name}</Title>}>
        {/* Modal content */}
        <Stack>
