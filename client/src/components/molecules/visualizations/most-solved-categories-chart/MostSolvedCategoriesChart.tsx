...
export default function MostSolvedCategoriesChart({ data }: any) {
    ...
    series: [
      {
        ...
        data:
          slicedData.length > 0
            ? slicedData.map(({ count }: { count: string }) => parseInt(count))
            : [],
      },
    ],
    options: {
      ...
      colors:
        slicedData.length > 0
          ? slicedData.map(({ name }: { name: string }) => stringToColour(name))
          : ["#fff"],
      theme: {
      ...
      xaxis: {
        type: "category",
        categories:
          slicedData.length > 0 ? slicedData.map(({ name }: { name: string }) => name) : [""],
        position: "bottom",
        axisBorder: {
          show: false,
        },
  ...
  return (
    ...
    <Paper shadow="xs" p="xl">
      ...
      <Modal
        ...
        title={<Title order={4}>Solved problems by category</Title>}
      >
        {/* Modal content */}
