...
import { Button, Modal, Select, Stack, TextInput, Title } from "@mantine/core"
...
export default function AdminUserEditModal({ data: user, open, setOpen }: AdminUserEditModalProps) {
  const client = useQueryClient()

  const mutation = useMutation((data) => udpateUserData(Number(user?.id), data), {
    onSuccess: ({ body }) => {
      client.invalidateQueries("users")
      if (user?.username !== body.username || user?.name !== body.name)
  ...
  return (
    <Modal
      ...
      title={<Title order={4}>Edit: {user?.name}</Title>}
    >
