...
export function createOperatorFilter<TRowValue, TFilterValue>({
  ...
  const operatorsMap = Object.fromEntries(
    operators.map((x) => [x.op, x])
  ) as Record<string, DataGridFilterOperator<TRowValue, TFilterValue>>;
  // builder filer function
  const filterFn: DataGridFilterFn<any, DataGridFilterState<TFilterValue>> = (
    row,
    columnId,
    filter
  ) => {
  ...
  filterFn.element = function Filter({
    ...
    const Operator = useMemo(
      () => operatorsMap[filter?.op]?.element,
      [filter?.op]
    );
