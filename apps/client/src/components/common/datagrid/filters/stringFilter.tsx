...
export const StringFilterInput: DataGridFilterInput<string> = ({
  onChange,
  ...rest
}) => (
  <TextInput
...
export const stringOperators = {
  includes: (label = "includes"): DataGridFilterOperator<string, string> => ({
    ...
    filterFn: (rowValue, filterValue) =>
      format(rowValue).includes(format(filterValue)),
    element: StringFilterInput
  }),
  notIncludes: (
    label = "not includes"
  ): DataGridFilterOperator<string, string> => ({
    op: "nin",
    label,
    filterFn: (rowValue, filterValue) =>
      !format(rowValue).includes(format(filterValue)),
  ...
  equals: (label = "equals"): DataGridFilterOperator<string, string> => ({
    ...
    label,
    filterFn: (rowValue, filterValue) =>
      format(rowValue) === format(filterValue),
    element: StringFilterInput
  }),
  notEquals: (
    label = "not equals"
  ...
  startsWith: (
    ...
    label,
    filterFn: (rowValue, filterValue) =>
      format(rowValue).startsWith(format(filterValue)),
    element: StringFilterInput
  }),
  endsWith: (label = "ends with"): DataGridFilterOperator<string, string> => ({
    ...
    label,
    filterFn: (rowValue, filterValue) =>
      format(rowValue).endsWith(format(filterValue)),
    element: StringFilterInput
  })
};

export function createStringFilter(
  options?: Partial<OperatorFilterOptions<string, string>>
) {
