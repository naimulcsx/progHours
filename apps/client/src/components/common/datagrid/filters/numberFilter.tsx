  ...
  Array.isArray(value) ? (
    <Group noWrap grow>
      ...
      <NumberInput
        {...rest}
        value={value[1]}
        onChange={(val) => onChange([value[0], Number(val) ?? 0])}
        hideControls
      />
    </Group>
  ) : (
...
const getLeftValue = (value: NumberFilterValue) =>
  Array.isArray(value) ? value[0] : Number.MIN_VALUE;
const getRightValue = (value: NumberFilterValue) =>
  Array.isArray(value) ? value[1] : Number.MAX_VALUE;

export const numberOperators = {
  equals: (
    label = "equals"
  ): DataGridFilterOperator<number, NumberFilterValue> => ({
    op: "eq",
    label,
  ...
  between: (
    ...
    filterFn: (rowValue, filterValue) =>
      rowValue > getLeftValue(filterValue) &&
      rowValue < getRightValue(filterValue),
    element: NumberFilterInput
  }),
  betweenOrEquals: (
    label = "between or equals"
  ): DataGridFilterOperator<number, NumberFilterValue> => ({
    op: "beteq",
    label,
    filterFn: (rowValue, filterValue) =>
...
export const initNumberFilterValue = (
  op: string,
  last?: NumberFilterValue
): NumberFilterValue => {
...
export function createNumberFilter(
  options?: Partial<OperatorFilterOptions<number, NumberFilterValue>>
) {
...
function createLeaderboardNumberFilter(
  options?: Partial<OperatorFilterOptions<number, NumberFilterValue>>
) {
  return createOperatorFilter<number, NumberFilterValue>({
    init: initNumberFilterValue,
...
export const leaderboardNumberFilterFn = createLeaderboardNumberFilter();
