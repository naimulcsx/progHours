...
const getLeftValue = (value: DateValue) =>
  Array.isArray(value) ? dayjs(value[0]) : dayjs(Number.MIN_VALUE);
const getRightValue = (value: DateValue) =>
  Array.isArray(value) ? dayjs(value[1]) : dayjs(Number.MAX_VALUE);

export function combineTimeAndDate(time: Date | null, date: Date | null) {
      ...
      return (
        <>
          <DatePickerInput
            ...
            onChange={(value) =>
              onChange([toString(value[0]), toString(value[1])])
            }
...
export const dateOperators = {
  equals: (
    ...
    filterFn: (rowValue, filterValue) =>
      dayjs(rowValue).isSame(
        getLeftValue(filterValue),
        withTime ? "minute" : "day"
      ),
  ...
  range: (
    ...
    filterFn: (rowValue, filterValue) => {
      ...
      return (
        getLeftValue(filterValue).isSameOrBefore(
          rowDate,
          withTime ? "minute" : "day"
        ) &&
        rowDate.isSameOrBefore(
          getRightValue(filterValue),
          withTime ? "minute" : "day"
        )
      );
    },
...
export const initDateFilterValue = (
  op: string,
  last?: DateValue
): DateValue => {
  if (op === "range") {
...
export function createDateFilter(
  options?: Partial<OperatorFilterOptions<string | Date | Dayjs, DateValue>>
) {
